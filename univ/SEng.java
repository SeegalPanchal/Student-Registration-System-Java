...
public class SEng extends HonoursDegree implements Serializable {
    ...
    public SEng() {
        super("SEng");
        super.readCourseCodeList("sengcourses.ser");
    }
    ...
    public boolean meetsRequirements(ArrayList<Course> allTheCoursesPlannedAndTaken) {      
        ...
        if (totalCredits < 20.00) { return false; }
        if (cisCredits < 11.25) { return false; }
        if (aOACredits < 4.00) { return false; }
        if (electives < 4.75) { return false; }
        if (aOA3000 < 1.00) { return false; }
        if (cis3000 < 6.00) { return false; }
        if (cis4000 < 2.00) { return false; }
        return true;
    }
